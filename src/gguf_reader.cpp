...
UrlDataSource::UrlDataSource(const std::string& url) : url(url), currentPos(0), abortDownload(false) {
    curl = curl_easy_init();
    if (!curl)
...
int UrlDataSource::ProgressCallback(void* clientp, curl_off_t, curl_off_t dlnow, curl_off_t, curl_off_t) {
    bool* abort_flag = static_cast<bool*>(clientp);
    return (*abort_flag) ? 1 : 0;
}

// GGUFMetadataReader implementation
